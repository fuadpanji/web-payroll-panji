...
let monthYear = month == '' ? new Date().toLocaleDateString('en-US', { year: 'numeric', month: '2-digit' }) : month + "-" + year
...
//         $(".date>.is-invalid").removeClass("is-invalid");
//         $(".date").find("#form-error").html("");
//     },
// });
...
$("#signin_time").flatpickr({
    ...
    noCalendar: true,
    onChange: (selectedDates, dateStr, instance) => {
        $(".signin_time>.is-invalid").removeClass("is-invalid");
        $(".signin_time").find("#form-error").html("");
    },
});
...
$(function () {
    ...
    if (dt_basic_table.length) {
        dt_basic = dt_basic_table.DataTable({
            ...
            ajax: {
                url: "attendance/datatable_attendance",
                type: "POST",
                data: {
                    empId: empId,
                    monthYear: monthYear
            ...
            columnDefs: [
                ...
                },
                {
                    ...
                    render: function (data, type, full, meta) {
                        let editBtn = ''
                        if(full[5] == "-") editBtn = `<button href="javascript:;" onclick="edit_data('${full[0]}')" class="btn btn-md btn-icon item-edit" title="Ubah"><i class="text-primary ti ti-pencil fs-4"></i></button>`
                        return (
                            editBtn
                            // +
            ...
            buttons: [
                {
                    text: '<i class="ti ti-plus me-sm-1"></i> <span class="d-sm-inline-block">Tambah Presensi Baru</span>',
                ...
                details: {
                    display: $.fn.dataTable.Responsive.display.modal({
                        header: function (row) {
                            var data = row.data();
                            return (
                                "Detail dari Presensi " + (data[2] != null ? data[2] : data[3])
                            );
                        },
                    }),
                    type: "column",
...
function submitForm() {
    ...
    $.ajax({
        url: "attendance/create_attendance",
        type: "POST",
        data: formData,
        dataType: "JSON",
        contentType: false,
...
function edit_data(id) {
    ...
    $.ajax({
        url: "attendance/update_attendance_form",
        type: "POST",
        dataType: "html",
        data: {
            id: id,
